// 01:35 START! 02:30 END! (총 55분 소요)
// 분할 정복? DP!
class Solution {
    // 💡 가장 먼저 규칙성 파악하기
    // n = 1일 때, (뚜껑이 있다면) 4가지 경우 / (뚜껑이 없다면) 3가지 경우
    //
    // n = 2일 때, (뚜껑이 있다면) 3가지 * (n = 1) / (뚜껑이 없다면) 2가지 * (n = 1)
    // ❗️ 둘이 연결되어 있다면? (n=1에 뚜껑이 있으면) +3가지 / (n=1에 뚜껑이 없으면) +2가지 ... A
    //
    // n = 3일 때, (뚜껑이 있다면) 3가지 * (n = 2) / (뚜껑이 없다면) 2가지 * (n = 2)
    // ❗️ 둘이 연결되어 있다면? ((n=2에 뚜껑이 있으면) 2가지 * (n=1) / (n=2에 뚜껑이 없으면) 1가지 * (n=1)) + A
