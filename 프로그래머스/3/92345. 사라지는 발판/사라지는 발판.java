...
class Solution {
    ...
    int[] dx = {-1, 0, 1, 0};
    int[] dy = {0, 1, 0, -1};
    public int play(int[][] board, int[] player, int[] waiting) {
        int x = player[0];
        int y = player[1];

        if(board[x][y] == 0) return 0;
        if(canNotMove(player, board)) return 0;
        ...
        for(int d = 0; d < 4; ++d) {
            ...
            if(nx >= 0 && nx < n && ny >= 0 && ny < m && board[nx][ny] == 1) {
                int count = 1 + play(board, waiting, new int[]{nx, ny});

                if(best % 2 == 0) { // 짝수면 A 입장에서 지는 것 / 원래 A가 지는 경우만 있었음
